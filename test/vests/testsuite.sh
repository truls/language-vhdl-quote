...
get_entity()
{
    if [ x$use_last_entity = x"no" ]; then
        ...
        sed -n -E -e '/^(ENTITY|entity) .*(ent|entw|cfg) (is|IS)$/s/(ENTITY|entity) (.*) (IS|is)/\2/p' \
            $1 | tail -1
    fi
}
...
test_ashenden() {
  ...
  dir=vhdl-93-transformed/clifton-labs/compliant
  . $dir/compliant1.exp

  # ashenden compliant
  do_inter_clean="no"
  # OK
  dir=vhdl-93-transformed/ashenden/compliant
  . $dir/compliant.exp
...
test_billowitch() {
  ...
  # OK.
  dir=vhdl-93-transformed/billowitch/compliant
  . $dir/compliant.exp
